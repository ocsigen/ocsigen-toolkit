...
let%shared make ?(a = []) ?(vertical = false) ?(position = 0)
  ...
  let swipe_pos_sig =
    [%client
      (React.S.create 0. : float React.S.t * (?step:React.step -> float -> unit))]
  in
      ...
      ( let add_transition = add_transition ~%transition_duration in
        ...
        let set_active () =
          ...
          List.iteri
            (fun i page ->
              ...
              if i >= pos && i < pos + React.S.value ~%nb_visible_elements then
                (* Page is visible *) Manip.Class.add page "ot-active")
            ~%pages
        in
        (********************** end *)
        (**********************
        ...
        *)
        ...
        let set_position ?transitionend pos =
          ...
          Lwt.async (fun () ->
              let%lwt () =
                if move then Lwt_js_events.transitionend d2'
        ...
        Lwt.async (fun () ->
            ...
            Eliom_client.onunload (fun () ->
                React.S.stop ~strong:true update_size);
            Lwt.return_unit);
        let perform_animation a =
          ~%set_nb_visible_elements (comp_nb_visible_elements ());
          if not (React.S.value ~%disabled) then (
            match (!action, a) with
            | `Change _, _ ->
            ...
            | _ ->
                action := a;
                if not !animation_frame_requested then (
        ...
        let compute_speed prev_speed prev_delta prev_timestamp delta =
          ...
          let speed =
            if delta_t = 0. then prev_speed
            else
              let cur_speed = (float delta -. float prev_delta) /. delta_t in
              if delta_t >= average_time then cur_speed
              else
        ...
        let onpan ev _ =
          ...
          | Start (startx, starty, prev_timestamp) ->
              ...
              status :=
                if
                  abs (if vertical then clX ev - startx else clY ev - starty)
                  >= abs move
                then Stopped
                  (* swiping in wrong direction (vertical/horizontal) *)
                else if abs move > Ot_swipe.threshold then (
                  (* We decide to take the event *)
                  (* We send a touchcancel to the parent
                     (which received the start) *)
                  Ot_swipe.dispatch_event ~ev d2' "touchcancel" (clX ev)
                    (clY ev);
                  Manip.Class.add ~%d2 ot_swiping;
                  set_top_margin ();
        ...
        let do_end ev startx starty prev_speed prev_delta prev_timestamp =
          ...
          status := Stopped;
          let width, delta =
            if vertical then (d2'##.offsetHeight, clY ev - starty)
          ...
          in
          if newpos <> pos then perform_animation (`Change newpos)
               ...
               React.E.map
                 (fun v ->
                   ...
                   | `Next ->
                       ...
                       if curpos < maxi then
                         perform_animation (`Change (curpos + 1))
                       else Lwt.return_unit
                   | `Prev ->
                       let curpos = Eliom_shared.React.S.value pos_signal in
                       if curpos > 0 then
...
let%shared default_fail_fun e =
  if Eliom_config.get_debugmode () then em [ txt (Printexc.to_string e) ]
  else
...
let%shared make_lazy ?a ?vertical ?(position = 0) ?transition_duration ?inertia
  ...
  let mk_contents : int -> 'gen -> ('a elt * ('a elt * 'gen) option ref) Lwt.t =
   fun i gen ->
    if i = position then generate_initial_contents ~spinner sleeper gen
    else
      Lwt.return
      @@
      let s = spinner () in
  ...
  let _ =
    [%client
      ( if ~%spinners_and_generators = [] then ()
        else
          let _ =
            ~%carousel.pos
            |> React.S.map @@ fun i ->
    ...
    [%client
      ( let add_transition = add_transition ~%transition_duration in
        ...
        Lwt.async (fun () ->
            ...
            Ot_noderesize.noderesize (Ot_noderesize.attach container')
              (fun () -> set_containerwidth container'##.offsetWidth);
            ...
            in
            Eliom_client.onunload (fun () ->
                React.S.stop ~strong:true watch_width);
              ...
              (React.S.l3
                 (fun pos size containerwidth ->
                   ...
                   | Some _, Some firstselectedelt, Some lastselectedelt ->
                       ...
                       let newleft =
                         if newleft > 0 then
                           max initial_gap ((containerwidth - ul_width) / 2)
                         else if newleft + ul_width < containerwidth then
                           min
                ...
                ignore
                  (React.S.l5
                     (fun pos offset size curleft _ ->
                       ...
                       in
                       let nextelt =
                         if pos + size < nb_pages then
                           Manip.nth the_ul (pos + size)
                         else None
                       in
        ...
        Ot_swipe.bind ~min:fmin ~max:fmax
          ~compute_final_pos:(fun ev p ->
            ...
            let pos =
              max (-the_ul'##.scrollWidth + (cw / 2)) (min (cw / 2) p)
            in
            (* Limit the movement
      ...
      :: a_onclick
           [%client
             fun _ ->
               ...
               ~%change
                 ( if offset > 1 then `Goto (React.S.value ~%pos - offset)
                 else `Prev )]
...
let%client bind_arrow_keys ?use_capture ?(vertical = false) ~change elt =
  Lwt_js_events.keydowns ?use_capture elt (fun ev _ ->
      ...
      if on_text_input then ()
      else if vertical then (
        if key = 40 (* down *) then change `Next
        else if key = 38 (* up *) then change `Prev )
      else if key = 39 (* right *) then change `Next
      else if key = 37 (* left *) then change `Prev;
      Lwt.return_unit)
...
let%shared wheel_make_transform z faces face_size ~vertical ?(delta = 0) pos =
  ...
  in
  if vertical then Printf.sprintf "translateZ(%dpx) rotateX(%.3fdeg)" (-z) angle
