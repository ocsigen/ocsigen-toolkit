...
let%client round x =
  if round_lb <= x && x <= round_ub then floor (x +. 0.49999999999999994) else x
...
let polar_to_cartesian (r, h) =
  let r = float_of_int r and h = float_of_int h in
...
let%client cartesian_to_polar (x, y) =
  let x = float_of_int (x - center_x) and y = float_of_int (y - center_y) in
...
let round_5_minutes ?(round_5 = false) m =
  if round_5
  then
    let m = m mod 60 in
    let m = if m mod 5 > 2 then (m / 5 * 5) + 5 else m / 5 * 5 in
    if m >= 60 then 55 else m
  else m

let round_30_degrees e =
  let e = e mod 360 in
  let e = if e mod 30 > 15 then (e / 30 * 30) + 30 else e / 30 * 30 in
  if e >= 360 then 330 else e

let angle_to_hours_minutes ?(round_5 = false) e =
  let m = e * 2 in
  let h = m / 60
  and m =
    let m = m mod 60 in
    if round_5 then if m mod 5 > 2 then (m / 5 * 5) + 5 else m / 5 * 5 else m
  in
  let m = if m >= 60 then if round_5 then 55 else 59 else m in
  assert (0 <= h && h <= 11);
  assert (0 <= m && m <= 59);
  h, m
...
let angle_to_minutes_round e =
  let m = e / 30 * 5 in
  assert (0 <= m && m <= 59);
  if e mod 30 >= 15 && m <= 50 then m + 5 else m

let angle_to_minutes ?(round_5 = false) e =
  if round_5 then angle_to_minutes_round e else angle_to_minutes e

let cartesian_of_i ?(radius = 40) de i =
  let e = i * de in
...
let string_of_hours ?(h24 = false) h =
  if h24
  then string_of_int h
  else if h = 0 || h = 12
  then "12"
  else if h < 12
  then string_of_int h
  else string_of_int (h - 12)

let display_hours_minutes_seq ?(h24 = false) f (h, m) b =
  let h' =
    let a =
      if b
      then [D.a_class ["ot-tp-hours"; "ot-tp-active"]]
      else
  ...
  and m =
    let a =
      ...
      else [D.a_class ["ot-tp-minutes"; "ot-tp-active"]]
    and c = [Printf.sprintf "%02d" m |> D.txt] in
    D.span ~a c
  and a = [D.a_class ["ot-tp-display"]] in
  if h24
  then D.div ~a [h'; D.txt ":"; m]
  else D.div ~a [h'; D.txt ":"; m; D.txt (if h < 12 then " AM" else " PM")]
...
let get_angle_signal ?round =
  Eliom_shared.React.S.map
    [%shared
      fun (e, b) ->
        match b, ~%round with
        | true, Some true -> round_30_degrees e
        | _, _ -> e]
      ...
      Eliom_shared.Value.create
        ...
        [%client
          fun ?step ((x, b) as p) ->
            ...
            if b then ~%f_b false]
    in
    clock_html_wrap_24h ~classes:["ot-tp-clock-hr"] (clock_svg_24h is_am e_h')
      f_e_h f_is_am
  in
  let g =
  ...
  let c, is_am =
    let update =
      ...
      | Some update ->
          ...
          Some
            [%client
              (React.E.map (fun (h, _) -> h < 12) ~%update : bool React.E.t)]
      | None -> None
    in
    ...
    and f_e_h =
      [%client
        fun ?step ((x, b) as p) ->
          ...
          if b then ~%f_b false]
    in
    clock_html_wrap ~classes:["ot-tp-clock-hr"]
      (clock_svg ~zero_is_12:true e_h')
      f_e_h
  in
  let g =
    let e_m = get_angle_signal ?round:round_5 e_m in
