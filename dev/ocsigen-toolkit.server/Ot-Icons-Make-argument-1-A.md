...
```ocaml
type +'a elt = 'a Eliom.Content.Html.elt
```
HTML elements.
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Ot-Icons-Make-argument-1-A-Xml.md) for details.
...
```ocaml
type +'a attrib = 'a Eliom.Content.Html.attrib
```
HTML attributes
...
module Xml : 
  Xml_sigs.T
    with type 'a W.t = 'a Eliom.Content.Xml.W.t
    with type 'a W.tlist = 'a Eliom.Content.Xml.W.tlist
    with type ('a, 'b) W.ft = ('a, 'b) Eliom.Content.Xml.W.ft
    with type uri = Eliom.Content.Xml.uri
    with type event_handler = Eliom.Content.Xml.event_handler
    with type mouse_event_handler = Eliom.Content.Xml.mouse_event_handler
    with type keyboard_event_handler = Eliom.Content.Xml.keyboard_event_handler
    with type touch_event_handler = Eliom.Content.Xml.touch_event_handler
    with type attrib = Eliom.Content.Xml.attrib
    with type elt = Eliom.Content.Xml.elt
```
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Ot-Icons-Make-argument-1-A-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Ot-Icons-Make-argument-1-A-Unsafe.md#val-string_attrib).
...
```ocaml
val svg : 
  ?a:[< Html_types.svg_attrib ] Eliom.Content.Svg.attrib list ->
  [< Html_types.svg_content ] Eliom.Content.Svg.elt list_wrap ->
  [> Html_types.svg ] elt
```
...
type ('a, 'b, 'c) lazy_star =
  ?a:'a attrib list ->
  'b elt list Eliom.Eliom_lazy.request ->
  'c elt
```
```ocaml
val lazy_form : 
...
val make_uri : 
  ...
  ?https:bool ->
  service:
    ('get, unit, Eliom.Service.get, _, _, _, _, _, _, unit, _) Eliom.Service.t ->
  ?hostname:string ->
  ?port:int ->
  ?fragment:string ->
  ?keep_nl_params:[ `All | `Persistent | `None ] ->
  ?nl_params:Eliom.Parameter.nl_params_set ->
  'get ->
  uri
```
...
If the CSS is generated by an Eliom service, use [`make_uri`](./#val-make_uri) to calculate the service URI. If the CSS is a static file, you may also use `Service.static_dir` or `Service.extern` to abstract the file with a service.
...
If the script content is generated by an Eliom service, use [`make_uri`](./#val-make_uri) to calculate the service URI. If it is a static file, you may also use `Service.static_dir` or `Service.extern` to abstract the file with a service.
...
module Form : 
  Eliom.Form_sigs.S
    with type +'a elt := 'a elt
     ...
     and type uri := uri
     and type 'a param = 'a Eliom.Content.Html.form_param
```
